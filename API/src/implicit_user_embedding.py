...
from API.src.model.evaluate import extract_prompt_embedding
import clip
import torch
...
def get_user_overall_embedding(image_folder_path, prompt, alpha, beta):
    ...
    if prompt:
        def extract_text_embedding():
            return extract_prompt_embedding(prompt)
    else:
...
if __name__ == "__main__":
    ...
    image_folder_path = os.path.join(SCRIPT_DIR, 'API/data/images')
